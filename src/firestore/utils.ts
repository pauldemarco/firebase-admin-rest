			...
			return {
				mapValue: {
					fields: Object.keys(value).reduce(
						(acc, k) => ({ ...acc, [k]: humanValueToDumbGoogle(value[k]) }),
						{}
					)
				}
			};
...
export function googleDumbObjectToHuman(inputObject: { [key: string]: FirestoreDocument }) {
	let fields: {
		[key: string]: any;
	} = {};
	for (const key in inputObject) {
...
export function googleDumbValueToHuman(inputObject: { [key: string]: any }) {
	for (const key in inputObject) {
		if (inputObject.hasOwnProperty(key)) {
			const value = inputObject[key];
			switch (key) {
