...
export async function DocsToJSONRest<T extends object>(
	...
	try {
		...
		if (!Array.isArray(finalJsonArr)) {
			throw new Error(
				`JSON payload in collection ${collectionPath} is corrupt, a collection using the DJ engine can't be used to store anything else but its own document data.`
			);
		}
		const finalJson: Document[] = JSON.parse(concattedJSONString || '[]').map(
...
export async function JSONtoDocsRest<T extends object>(
	...
	try {
		...
		const writePromises = jsonArr.map((jsonPart, index) => {
			return setDocRest(
				`${collectionPath}/JSON_STRING_${index}`,
				{
...
export async function DJDeleteREST(
	collectionPath: string,
	options: {
		db?: string;
	}
